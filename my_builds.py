...
def add_official_approval_test_repos(builds):
    ...
    builds.add_source_repo('approvals', repo, ['master'], TravisConfig(False), AppveyorConfig('lf3i76ije89oihi5', 'isidore'))
    builds.add_source_repo('claremacrae', repo, ['master'], TravisConfig(True),
                           AppveyorConfig('37smtsp3a694okv8'))  # 'setup_sanitizers'

    repo = 'ApprovalTests.cpp.StarterProject'
    builds.add_source_repo('approvals', repo, ['master'], TravisConfig(False), AppveyorConfig('qx0546k6ii57919w', 'isidore'))
    builds.add_source_repo('claremacrae', repo, ['master'], TravisConfig(True), AppveyorConfig('ytjgybf5r9fviifm'))

    repo = 'ApprovalTests.cpp.Qt'
    builds.add_source_repo('approvals', repo, ['master'], TravisConfig(True), AppveyorConfig('pf8et0nk1mdajskf', 'isidore'))
    builds.add_source_repo('claremacrae', repo, ['master'], TravisConfig(True), AppveyorConfig('g60qbttap7m5nul2'))

    repo = 'ApprovalTests.cpp.Qt.StarterProject'
    builds.add_source_repo('approvals', repo, ['master'], TravisConfig(False), AppveyorConfig('tpitsul9axlv93uk', 'isidore'))
    builds.add_source_repo('claremacrae', repo, ['master'], TravisConfig(True), AppveyorConfig('xe2iwuto0sc342a7'))
...
def add_my_random_repos(builds):
    ...
    builds.add_source_repo('claremacrae', 'ci_playground', ['trunk'], TravisConfig(True), AppveyorConfig('cbksrgvypq5vksy2'))
    builds.add_source_repo('claremacrae', 'cpp_snippets', ['master'], TravisConfig(True), AppveyorConfig('hqf8xh615dyp3u4l'))


def add_boost_ut_repos(builds):
    repo = 'ut'
    builds.add_source_repo('boost-experimental', repo, ['master'], TravisConfig(False))
    builds.add_source_repo('claremacrae', repo, ['master', 'clare_learning'], TravisConfig(True),
